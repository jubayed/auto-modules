...
namespace Jubayed\Composer\Autoload;
...
class AutoloadGenerator
{
     ...
     */
    public function dump(Config $config, InstalledRepositoryInterface $localRepo, $composer, InstallationManager $installationManager, string $targetDir, bool $scanPsrPackages = false, ?string $suffix = null)
    {
        $rootPackage = $composer->getPackage();

        $this->module = basename($targetDir);
        $this->website = basename(dirname($targetDir));

        $extraPackage = new ExtraPackage($composer, $this->website, $this->module);
        $extraPackage->mergeInto($rootPackage);        
...
EOF;
        ...
        file_put_contents($targetDir.'/ClassLoader.php', file_get_contents(__DIR__.'/ClassLoader.stub'));
        // die ();
        // $filesystem->safeCopy(__DIR__.'/ClassLoader.php', );
...
PLATFORM_CHECK;
    ...
    protected function getAutoloadFile(string $vendorPathToTargetDirCode, string $suffix)
    {
        ...
        // $real_path = dirname($vendorPathToTargetDirCode, 3). '/{$website}/{$module}/autoload_real.php';

        return <<<AUTOLOAD
<?php

// autoload.php @generated by jubayed saas system

if (PHP_VERSION_ID < 80100) {
    if (!headers_sent()) {
        header('HTTP/1.1 500 Internal Server Error');
    }
    \$err = 'Composer 2.3.0 dropped support for autoloading on PHP <8.1 and you are running '.PHP_VERSION.', please upgrade PHP or use Composer 2.2 LTS via "composer self-update --2.2". Aborting.'.PHP_EOL;
    if (!ini_get('display_errors')) {
        if (PHP_SAPI === 'cli' || PHP_SAPI === 'phpdbg') {
            fwrite(STDERR, \$err);
        } elseif (!headers_sent()) {
...
function api_res(\$message, \$code = 403){
...
if( isset(\$_SERVER['REQUEST_URI']) == false){
    api_res('Module path are required');
}

if(substr(\$_SERVER['REQUEST_URI'], 0, 8) != '/module/'){

    api_res('Module path are required');
}

\$module = substr(\$_SERVER['REQUEST_URI'], 8, 20);
\$_SERVER['MODULE_NAME'] = substr(\$module, 0, strpos(\$module, "/"));

if(is_null(\$_SERVER['MODULE_NAME']) || \$_SERVER['MODULE_NAME'] == ''){

    api_res('Module path are required');
}

\$real_path = __DIR__ . "/jubayed/autoload/{\$_SERVER['MODULE_NAME']}/autoload_real.php";

if(!file_exists(\$real_path)){

    api_res('Autoload file not generated. ~/composer/autoload_real.php');
}
